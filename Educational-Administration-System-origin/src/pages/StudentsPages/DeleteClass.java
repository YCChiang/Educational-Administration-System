...
public class DeleteClass extends JFrame {
				...
				if(e.getSource() == button)
				{
					...
					for(int i = 0; i < selectedRows.length; i++)
					{
						...
						if(msg.isError()) {
							...
							JOptionPane.showMessageDialog(null, msg.getContent(), "错误提示", JOptionPane.ERROR_MESSAGE);
						}
						else {
							// TODO 提示弹窗，消息为msg.getContent()	
							JOptionPane.showMessageDialog(null, msg.getContent(), "提示信息", JOptionPane.ERROR_MESSAGE);
	...
	protected void  myWindowListener(WindowEvent e)
	{
		...
		if(!classInfo.isEmpty()) {
			for(ClassInfo c:classInfo) {
				// 将数据添加到表格中
				tableModel.addRow(new Object[] {c.getId(),c.getName(),c.getTeacher_name(),c.getCapacity(),c.getYear(),c.getStart_week()+"-"+c.getEnd_week()});
			}			
		}
