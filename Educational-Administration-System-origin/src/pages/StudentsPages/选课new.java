		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (e.getSource() == button) {
					...
					if (input == null) {
						JOptionPane.showMessageDialog(null, "请输入查询内容", "错误提示", JOptionPane.ERROR_MESSAGE);
					} else {
						// System.out.println(id);
						// System.out.println(name_t);
						if (id && !name_t) {
							// System.out.println(1);
							classInfo = classinfoservice.seachClassById(input, "2020春", user.getname(), IsFull,
									IsConflict);
							// if(classInfo == null )
							...
							classInfo = classinfoservice.seachClassByName(input, "2020春", user.getname(), IsFull,
									IsConflict);
						}
						...
						if (!(classInfo.isEmpty())) {
							for (ClassInfo c : classInfo) {
								// 将数据添加到表格中
								tableModel.addRow(new Object[] { c.getId(), c.getName(), c.getTeacher_name(),
										c.getCapacity(), c.getYear(), c.getStart_week() + "-" + c.getEnd_week() });
							}
						}
