	...
	public void actionPerformed(ActionEvent ev)// 为按钮添加的注册事件
	{
		if (ev.getSource() == jb1) {
			if (jcb1.isSelected() || jcb2.isSelected() || jcb3.isSelected()) {
				...
				loginuser.setname(jtf1.getText());
				loginservice L = new loginservice();
				User get = L.findById(loginuser);
				if (get == null) {
					System.out.println(jlb1.getText());
					JOptionPane.showMessageDialog(null, "此用户不存在", "错误提示", JOptionPane.ERROR_MESSAGE);
				} else {
					if (jcb1.isSelected()) {
				...
				JOptionPane.showMessageDialog(null, "请选择身份", "错误提示", JOptionPane.ERROR_MESSAGE);
			}
